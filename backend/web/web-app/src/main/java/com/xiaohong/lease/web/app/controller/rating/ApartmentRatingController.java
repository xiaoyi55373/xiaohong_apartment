package com.xiaohong.lease.web.app.controller.rating;
 ...
 */
@Tag(name = "公寓评分", description = "公寓多维度评分相关接口")
@RestController
@RequestMapping("/app/rating")
public class ApartmentRatingController {

    private static final Logger log = LoggerFactory.getLogger(ApartmentRatingController.class);
    ...
    @BehaviorLog(value = BehaviorType.SUBMIT, desc = "提交公寓评分", targetType = "rating", targetId = "#submitVo.apartmentId")
    @PostMapping("/submit")
    public Result<Void> submitRating(@RequestBody RatingSubmitVo submitVo) {
        Long userId = LoginUserContext.getLoginUser().getUserId();
        log.info("提交评分, userId: {}, apartmentId: {}", userId, submitVo.getApartmentId());
    ...
    public Result<RatingStatisticsVo> getRatingStatistics(
        ...
        log.info("获取评分统计, apartmentId: {}", apartmentId);
    ...
    public Result<IPage<RatingItemVo>> pageRatingItem(
        ...
        log.info("获取评分列表, apartmentId: {}, current: {}, size: {}", apartmentId, current, size);
    ...
    @BehaviorLog(value = BehaviorType.SUBMIT, desc = "删除公寓评分", targetType = "rating", targetId = "#ratingId")
    @DeleteMapping("/delete/{ratingId}")
    public Result<Void> deleteRating(@Parameter(description = "评分ID") @PathVariable Long ratingId) {
        Long userId = LoginUserContext.getLoginUser().getUserId();
